...
#[tokio::test]
async fn thread_fork_persists_branch_context_for_resume() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
...
#[tokio::test]
async fn thread_fork_latest_assistant_anchor_truncates_latest_assistant_message() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    ...
    writeln!(
        ...
        json!({
            ...
            "payload": {
                "type":"message",
                "role":"assistant",
                "content":[{"type":"output_text","text": "latest branch line"}]
            }
        })
    )?;
    ...
    let fork_id = mcp
        .send_thread_fork_request(ThreadForkParams {
            thread_id: conversation_id,
            snapshot: Some(ThreadForkSnapshot::LatestAssistantReadAnchor {
                source_line_index: 0,
    ...
    let fork_rollout = std::fs::read_to_string(thread.path.expect("fork path"))?;
    assert!(
        fork_rollout.contains("latest"),
        "fork rollout should contain truncated assistant text: {fork_rollout}"
    );
    assert!(
        !fork_rollout.contains("branch line"),
        "fork rollout should drop unread assistant suffix: {fork_rollout}"
    );
