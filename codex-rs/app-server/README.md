...
- `thread/fork` — fork an existing thread into a new thread id by copying the stored history; if the source thread is currently mid-turn, the fork records the same interruption marker as `turn/interrupt` instead of inheriting an unmarked partial turn suffix. `thread/fork` also accepts an experimental `snapshot` policy. The default `interrupted` snapshot preserves current behavior, while `assistantReadAnchor` truncates a selected assistant message at a stable read anchor before appending the interruption marker for the new branch. Branching clients may also provide experimental `branchDepth`, `branchAnchorHeadSummary`, and `branchAnchorSummary` metadata, which is persisted on the forked thread and returned on resume/read. When the snapshot is an assistant read anchor, the returned thread also includes `branchOriginSnapshot` so clients can place the branch marker back into the parent transcript later. The head summary labels the beginning of the selected anchor; the anchor summary labels the cutoff point. The returned `thread.forkedFromId` points at the source thread when known. Accepts `ephemeral: true` for an in-memory temporary fork, emits `thread/started` (including the current `thread.status`), and auto-subscribes you to turn/item events for the new thread.
